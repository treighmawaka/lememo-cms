 ...
 * new chronology_events collection -- not part of the Directus runtime. Same
 * permission shape as the original 7 collections (see access.ts): Editor
 * gets full CRUD, Staff Writer gets create/read/update (no delete), Public
 * gets read.
 *
 * Idempotent: skips permissions that already exist.
 *
 * Usage:
 *   set -a && source .env && set +a && pnpm exec tsx schema/accessFlatChronologie.ts
 */
...
const NEW_COLLECTIONS = ['chronology_events']
