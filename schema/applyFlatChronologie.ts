/**
 * Creates the flat chronology_events collection -- not part of the Directus
 * runtime. Replaces chronologie's nested items[].events[] design: chronologie
 * stays the thin page-level parent (title, description, image, country,
 * years, SEO); chronology_events holds one row per timeline event,
 * individually queryable/sortable, relating back via the `chronologie` field.
 *
 * Idempotent: skips collection/fields/relation that already exist.
 *
 * Usage:
 *   set -a && source .env && set +a && pnpm exec tsx schema/applyFlatChronologie.ts
 */
import { createCollection, createField, createRelation, readCollection, readFieldsByCollection } from '@directus/sdk'

import { chronologyEventsCollection } from './collections/chronologyEvents'
import { getClient, login, primaryKeyField, type CollectionDef } from './lib'

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type AnyClient = any

const collections: CollectionDef[] = [chronologyEventsCollection]

const relations: { collection: string; field: string; related: string }[] = [{ collection: 'chronology_events', field: 'chronologie', related: 'chronologie' }]
